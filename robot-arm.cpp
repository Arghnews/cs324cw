...
float g_theta = 0.0f;
float g_phi = 0.0f;
float g_elbow_angle = 30.0f;
float g_wrist_angle = 30.0f;
float g_hand_angle = 40.0f;
float g_hand_angleX = 0.0f;
...
void display()
{
		...
		glPushMatrix();
			...
			glPushMatrix();
				...
				glPushMatrix();
					...
					glRotatef(g_wrist_angle,0,0,-1);
					glRotatef(g_hand_angle,0,1,0);
					glRotatef(g_hand_angleX,1,0,0);
    ...
    glDisable(GL_LIGHTING);
        ...
        draw_text(20, 880, "e/E - elbow, w/W - wrist, h/H - hand");
        draw_text(20, 840, "Use 'l'/'L' to change hand X angle");
    glEnable(GL_LIGHTING);
...
void keyboard(unsigned char key, int, int)
{
	switch (key)
	{
		...
		case 'H': g_hand_angle -= 5.0f; break;
		case 'l': g_hand_angleX += 5.0f; break;
		case 'L': g_hand_angleX -= 5.0f; break;
	}
	glutPostRedisplay();
}
